...
func genMysqlUpdate(schema string, table *model.TableInfo, row []byte, canAppendDefaultValue bool) (names []string, values []interface{}, oldValues []interface{}, err error) {
	columns := writableColumns(table)
	updtDecoder := newUpdateDecoder(table, canAppendDefaultValue)
		...
		for _, mut := range pv.GetMutations() {
			...
			for {
				...
				case tipb.MutationType_Update:
					names, args, oldArgs, err := genMysqlUpdate(schema, info, row, canAppendDefaultValue)
					if err != nil {
