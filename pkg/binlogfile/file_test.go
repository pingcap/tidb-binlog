...
func (t *testFileSuite) TestParseBinlogName(c *C) {
	cases := []struct {
		...
		{"binlog-0000000000000003-20180315121212-000000000000000001", 0000000000000003, 1, false},
		{"binlog-0000000000000003-20180315121212-000000000000000001.tar.gz", 0000000000000003, 1, false},
		{"binlog-index-20180315121212-000000000000000001", 0, 0, true},
	...
	index := uint64(1)
	name := BinlogName(index, 0)
	gotIndex, _, err := ParseBinlogName(name)
	c.Assert(err, IsNil)
